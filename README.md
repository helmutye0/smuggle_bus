...
- Content Bordering within Smuggle Bus (rather than simply appending contraband files to the end of the mask file, it would be useful to be able to place them inside the mask file, with mask file content before and after--currently the archive mode zip file and password are kept in such a "pocket" between the end of the mask file and the beginning of the contraband file, but this method can likely be expanded upon to create a similar "pocket" for the entire contraband file; an added advantage of this is that the content bordering would not have to be based on contraband file length but could rather be completely random--the beginning and end of the contraband file would be based not on number of bytes but on the content borders)
